...
const CartItem = ({item}) => {
  ...
  return (
    <div className='flex items-center gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500'>
      {/* product image */}
      <div className='pr-5 min-h-[150px] flex items-center gap-x-4'>
        <Link to={`/products/${id}`} >
      ...
      <div className='w-full flex flex-col'>
        <div className='flex justify-between mb-2'>
          <Link to={`/product/${id}`} className='text-sm uppercase font-medium text-primary hover:underline'>
            {title}
          </Link>
